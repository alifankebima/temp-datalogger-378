...
const fetchDownsampledData = () => {
    ...
    sampled_data AS (
        SELECT *,
               ...
               (SELECT COUNT(*) FROM tempdata t2 WHERE t2.created_at <= t1.created_at AND deleted_at IS NULL) AS row_num
        FROM tempdata t1
    )
    SELECT t1, t2, t3, t4, created_at as timestamp FROM (
        ...
        WHERE (row_num % CASE WHEN total_records > 50 THEN total_records / 48 ELSE 1 END = 0)
...
const softDeleteAllData = () => {
    ...
    return sqlite.executeQuery(`UPDATE tempdata SET deleted_at=${deleted_at} WHERE deleted_at IS NULL;`)
}
